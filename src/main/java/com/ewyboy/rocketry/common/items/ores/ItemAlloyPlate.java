...
public class ItemAlloyPlate extends ItemBase {
    ...
    public void registerItemRenderer() {
        for (int i = 0; i < EnumAlloys.values().length; i++) {
            if (EnumAlloys.byMeta(i).isTypeSet(EnumOreType.PLATE)) {
                ModelLoader.setCustomModelResourceLocation(this, i, new ModelResourceLocation(Reference.ModInfo.ModID + ":ores/plate-" + EnumAlloys.byMeta(i).getUnlocalizedName(), "inventory"));
            }
        }
    }
}